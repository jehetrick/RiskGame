...
#PLAYER variables
PLAYER_NAME = "Lyra" # change this to your name!
FRIEND1_NAME = "Makkiah" # change this to a friend's name!
FRIEND2_NAME = "Angie" # change this to another friend's name!
current_room = 31 # start room = 31

# Set this to start in a different room
#current_room = 41
...
#air, energy = 100, 100
financial, physical, social = 100, 100, 100
...
### Social Monster: 49
    49: [images.soc_monster_60x120, None, "Social Monster"],
#    49: [images.drone, None, "A delivery drone"],
### Physical Monster: 50
    50: [images.phys_monster_60x120, None, "Physical Monster - dangerous!"],    
### Financial Monster: 51
    51: [images.fin_monster_60x120, None, "Financial Monster - dangerous!"],
...
#         [48,3,4], [48,4,2], [48,4,3], [48,4,4]],
    33: [[13,1,1], [13,1,3], [13,1,8], [13,1,10]],
#         [48,2,7], [48,3,6], [48,3,3]], [48,2,1],
...
win_msg_social = [
"Congratulations! You made it to the rocket! Unfortunately, when you made it home you learned that your",
"spouse left you and your friends hate you. But at least you\'re healthy and financially stable."]

lose_msg_social = [
    "Oh no! Your teammates got to the rocket first! For anyone else they would have waited,",
    "but they don\'t like you. So, they left without you."]

win_msg_physical = [
"Congratulations! You made it to the rocket! Unfortunately, you were injured during your escape and you", 
"died upon entering the atmosphere. But at least your friends like you and you\'re financially stable."]

lose_msg_physical = [
    "Oh no! While trying to escape the space station,",
    "you sustained too many injuries and died."]

win_msg_financial = [
"Congratulations! You made it to the rocket! Unfortunately, when you made it back home you had", 
"to sell your house and live on the streets. But at least your friends like you and you\'re healthy."]

lose_msg_financial = [
    "Oh no! You ran out of money while trying to escape the space station. The only food",
    "available is in the vending machine, and now that that\'s off limits, you\'ll starve."]


## Ouch messages when bitten by monster

# Social
ouch_soc = [
'Your teammates are talking behind your back.',
'You tripped and made a fool of yourself.',
'A friend posted an embarrassing photo of you.',
'You spilled coffee all over your colleague.',
'You offended someone.',
'You gave someone a gift they didn\'t like.',
'You called someone by the wrong name.',
'You sneezed on someone.',
'You accidentally broke someone\'s stuff.',
'You forgot a friend\'s birthday.']

# Physical
ouch_phys = [
'You fell and scraped your knees.',
'You hit your head, hard.',
'You got a paper cut. Ouch.',
'You sprained your ankle.',
'You stubbed your toe.',
'You got your hand jammed in a door.',
'You got food poisoning.',
'A robot malfunctioned and slapped you.',
'You fell off the top of the rover.',
'You were mildly electrocuted.']

# Financial
ouch_fin = [
'You accidentally dropped $50.',
'You fell for a scam.',
'You lost your wallet.',
'You dropped your phone & had to replace it.',
'Even in space you had to pay taxes.',
'You paid for your friend\'s dinner.',
'A space crab stole your credit card.',
'$500 blew away.',
'You donate $2,000 to charity to clear your conscience.',
'You spent a little too much while celebrating your birthday.']
...
### Debug
    ...
    # hazard_map is now a (room_width, room_height) array of 0's    
...
def start_room():
    ...
    show_text("You are here: " + room_name, 0, GRAY)
...
def game_loop():
    ...
    global financial, physical, social
    global HELP
...
#    print('player is at: x=',player_x,' y=',player_y)
...
## Not Working Yet
  ...
  # Add a help page
    if keyboard.h and HELP==0:
        HELP = 1
        box = Rect((50, 50), (700, 500))
        screen.draw.filled_rect(box, BLACK)
        screen.draw.text("Keyboard Commands:", (100, 100), color=WHITE)
        screen.draw.text("\tleft,right,up,down arrows :  move", (100, 100), color=WHITE)
        screen.draw.text("\tg :   Pick up (get) object", (100, 150), color=WHITE)
        screen.draw.text("\tTAB :   Cycle through objects in my pack", (100, 200), color=WHITE)

    
    if keyboard.h and HELP==1:
        HELP = 0
        generate_map()
        start_room()

  # DEBUG: Display Risk Ouchies
    if keyboard.o:
        show_text("You hit a RISK Monster! and it really really hurts", 0, BLUE)
...
################################
  ...
  # If the player is standing somewhere they shouldn't, move them back.
    ...
    if room_map[player_y][player_x] == 48: # toxic floor
        # toxic floor penaty:
        toxic_floor_penalty = 0.1
        deplete_energy(toxic_floor_penalty)
...
#    screen.draw.filled_rect(box, RED)
    ...
    # added JH--so the the Vibes bars update
    draw_vibes()
...
def show_text(text_to_show, line_number, c):
    if game_over:
        return
    text_lines = [15, 50]
    ...
    screen.draw.text(text_to_show,
                     (20, text_lines[line_number]), color=c)
...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text("Now carrying " + objects[item_player_is_on][3], 0, WHITE)
        sounds.pickup.play()
        time.sleep(0.5)
    else:
        show_text("You can't carry that!", 0, WHITE)
...
def drop_object(old_y, old_x):
    ...
    if room_map[old_y][old_x] in [0, 2, 39]: # places you can drop things
        ...
        show_text("You have dropped " + objects[item_carrying][3], 0, WHITE)
        sounds.drop.play()
        remove_object(item_carrying)
        time.sleep(0.5)
    else: # This only happens if there is already a prop here
        show_text("You can't drop that there.", 0, WHITE)
        time.sleep(0.5)
...
Rendezvous Sector 13, outside."
        ...
        show_text("Congratulations, "+ PLAYER_NAME +"!", 0, WHITE)
        show_text("Mission success! You have made it to safety.", 1, WHITE)
        game_over = True
        ...
        show_text("Door to engineering bay is open for 60 seconds", 1, WHITE)
    ...
    if suit_stitched and air_fixed: # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            show_text("The computer tells you the airlock is now open.", 1, GREEN)
        elif props[20][0] == 31:
...
# from: https://learning.oreilly.com/library/view/python-cookbook/0596001673/ch02s12.html
def qsort(L):
    if len(L) <= 1: return L
...
def game_completion_sequence():
    ...
    if launch_frame < 9:
        ...
        screen.draw.text("MISSION", (200, 380), color = "white",
                     fontsize = 64, shadow = (1, 1), scolor = "black")
...
# Vibes
        vibe_color = [BLUE, RED, GREEN]
        win_msg = [win_msg_social, win_msg_physical, win_msg_financial]
        
        low_vibe_value = 1000
        for i,v in enumerate([social, physical, financial]):
            if(v < low_vibe_value):
                low_vibe_value=v
                vibe=i
       
        show_text(win_msg[vibe][0], 0, vibe_color[vibe])
        show_text(win_msg[vibe][1], 1, vibe_color[vibe])
        game_over = True
        sounds.win_lyra.play()
#        sounds.gameover.play()
        ...
        screen.draw.text(win_msg[vibe][1], (60, 330), color = WHITE,
                     fontsize = 24, shadow = (1, 1), scolor = vibe_color[vibe])
...
def shut_engineering_door():
    ...
    if current_room == 32:
        close_door(25)
    show_text("The computer tells you the doors are closed.", 1, WHITE)
...
#####################################
## Finanncial, Physical, Social Vibes   
#####################################
                      
def draw_vibes():
    global social, physical, financial
    box = Rect((20, 565), (640, 20))
    screen.draw.filled_rect(box, BLACK)
    screen.draw.text("FINANCIAL:", (20, 566), color=GREEN)
    screen.draw.text("PHYSICAL:", (250, 566), color=RED)
    screen.draw.text("SOCIAL:", (480, 566), color=BLUE)

    if financial > 0:
        box = Rect((120, 565), (financial, 20))
        screen.draw.filled_rect(box, GREEN) # Draw new financial bar.

    if physical > 0:
        box = Rect((350, 565), (physical, 20))
        screen.draw.filled_rect(box, RED) # Draw new physical bar.

    if social > 0:
        box = Rect((560, 565), (social, 20))
#        box = Rect((560, 565), (10, 20))
        screen.draw.filled_rect(box, BLUE) # Draw new social bar.


# We don't use this anymore
def end_the_game(reason):
    global game_over
    show_text(reason, 1, GREEN)
    game_over = True
...
def loose_the_game(vibe, reason1, reason2):
    global game_over

    vibe_color = [BLUE, RED, GREEN]
    lose_msg = [lose_msg_social, lose_msg_physical, lose_msg_financial]
    show_text(reason1, 0, vibe_color[vibe])
    ...
    screen.draw.text(lose_msg[vibe][0], (60, 300), color = WHITE,
                     fontsize = 24, shadow = (1, 1), scolor = vibe_color[vibe])
    screen.draw.text(lose_msg[vibe][1], (60, 330), color = WHITE,
                     fontsize = 24, shadow = (1, 1), scolor = vibe_color[vibe])
...
def alarm():
    show_text("Air is running out, " + PLAYER_NAME
              + "! Get to safety, then radio for help!", 1, GREEN)
    sounds.alarm.play(3)
...
def deplete_social(penalty):
    global social, game_over
    if game_over:
        return # Don't sap energy when they're already dead.
#    penalty = 10
    social -= penalty
    ...
    print("social:", social)
    if social < 1:
        loose_the_game(0, lose_msg_social[0], lose_msg_social[1])

def deplete_physical(penalty):
    global physical, game_over
    if game_over:
        return # Don't sap energy when they're already dead.
#    penalty = 10
    physical -= penalty
#    draw_vibes()
    ...
    if physical < 1:
        loose_the_game(1, lose_msg_physical[0], lose_msg_physical[1])

def deplete_financial(penalty):
    global financial, game_over
    if game_over:
        return # Don't sap energy when they're already dead.
#    penalty = 10
    financial -= penalty
    ...
    if financial < 1:
        loose_the_game(2, lose_msg_financial[0], lose_msg_financial[1])
...
def hazard_start():
    ...
    if current_room in hazard_data.keys():
        ...
        for hazard in current_room_hazards_list:
            hazard_y = hazard[0]
            hazard_x = hazard[1]
### THIS CODE Decides which monster lives in which room.
#            hazard_map[hazard_y][hazard_x] = 49 + (current_room % 3)
            hazard_map[hazard_y][hazard_x] = 49 + random.randint(0,2)
            current_monster = hazard_map[hazard_y][hazard_x]
            print("choose monster: rm =", current_room, "m =", hazard_map[hazard_y][hazard_x])
...
def hazard_move():
    ...
    for hazard in current_room_hazards_list:
        ...
        ### Save the hazard map info, because it contains which monster is in this room.
        old_haz_map = hazard_map[old_hazard_y][old_hazard_x]
        current_monster = old_haz_map
...
### Human interaction here with monster       
### Need to fix deplete_(vibe) here
...
##################################
            ...
            if(current_monster==49):
                sounds.ouch2.play()
                show_text(ouch_soc[random.randint(0,9)], 0, BLUE)
                deplete_social(hazard_penalty)
            elif(current_monster==50):
                sounds.ouch1.play()
                show_text(ouch_phys[random.randint(0,9)], 0, RED)
                deplete_physical(hazard_penalty)
            elif(current_monster==51):
                sounds.ouch4.play()
                show_text(ouch_fin[random.randint(0,9)], 0, GREEN)
                deplete_financial(hazard_penalty)
...
### Recalculates which monster is in this room (?)            
#        hazard_map[hazard_y][hazard_x] = 49 + (current_room % 3)
        hazard_map[hazard_y][hazard_x] = current_monster
...
#    screen.draw.filled_rect(box, BLACK)
#    screen.draw.text("Keyboard Commands:", (100, 100), color=WHITE)
#    screen.draw.text("\tleft,right,up,down arrows :  move", (100, 200), color=WHITE)
#    screen.draw.text("\tg :   Pick up (get) object", (100, 250), color=WHITE)
#   screen.draw.text("\tTAB :   Cycle through objects in my pack", (100, 300), color=WHITE)
